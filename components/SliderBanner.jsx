...
var SliderBanner = React.createClass({
  ...
  getInitialState: function() {
    ...
    touchEndFn:function(e){
      ...
      if(this.state.m>50){
        this.setState({m:0,idx:this.idxFn(this.state.idx-1,len),oldIdx:this.state.idx,end:true,left:!this.state.left});
      }else if(this.state.m<-50){
        this.setState({m:0,idx:this.idxFn(this.state.idx+1,len),oldIdx:this.state.idx,end:true,left:!this.state.left});
      }else {
      this.setState({m:0,idx:this.idxFn(this.state.idx,len),end:true});
      }
    },
    render:function() {
      ...
      return (
         <div id="slider" className="slider" onTouchStart={this.touchStartFn} onTouchMove={this.touchMoveFn}  onTouchEnd={this.touchEndFn}>
            ...
            { 
              ...
              var cont=obj.cont?obj.cont:'';
                return <div  className="slider-item ub-fh ub-fv ub-img1" onClick="" style={style} >{cont}</div>;
            },this)
          }
                </div>
              ...
              this.props.imgList.map( function (res,i) {
                  return <li className={the.state.idx==i?'active':''}></li>;
              })
            }
                </ul>
            </div>
